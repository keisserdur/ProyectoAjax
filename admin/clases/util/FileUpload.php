...
class FileUpload {
    ...
    private $destino = "./img/activity/", $extension, $nombre = "", $parametro, $size = 1000000;
    ...
    function __construct($parametro) {
        if (isset($_FILES[$parametro]) && $_FILES[$parametro]["name"] !== "") {
            ...
            $nombre = strtolower($_FILES[$this->parametro]["name"]);
            $trozos = pathinfo($nombre);
            $this->nombre = strtolower($trozos["filename"]);
            $this->extension = strtolower($trozos["extension"]);
        } else {
            $this->error = true;
        }
    }
